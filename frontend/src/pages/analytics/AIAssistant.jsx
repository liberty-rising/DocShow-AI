...
const AIAssistant = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}chat/analytics/`, {
        chat_id: chatId,
        prompt: userInput,
      });
      const llmOutput =
      ...
      setChatHistory((prevHistory) => [
        ...prevHistory,
        { role: "AI", message: llmOutput },
      ]);
    } catch (error) {
      setChatHistory((prevHistory) => [
        ...prevHistory,
        { role: "AI", message: "An error occurred." },
      ]);
    }
  ...
  return (
    <Box>
      ...
      <Button variant="contained" color="error" onClick={handleDeleteHistory}>
        Delete History
      </Button>
    </Box>
  );
